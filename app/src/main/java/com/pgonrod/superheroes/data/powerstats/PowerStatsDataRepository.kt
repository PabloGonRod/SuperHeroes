...
class PowerStatsDataRepository(
    ...
    override suspend fun getPowerStats(heroId: Int): Either<ErrorApp, Powerstats?> {
        val powerstats = localsource.getPowerstats(heroId)
        return if (powerstats.isLeft()){
            remotesource.getPowerStats(heroId).map { superheroes ->
                localsource.savePowerstats(heroId, superheroes!!)
                superheroes
            }
        } else{
            powerstats
